...
/// An embedded, Cortex-M task pool for polling futures to completion, based
/// the on `LocalPool` executor from futures-rs.
///
...
/// [`spawner()`](CortexMExecutor::spawner) method. Because the executor is
/// single-threaded, it supports a special form of task spawning for non-`Send`
/// futures, via [`spawn_local_obj`](futures_task::LocalSpawn::spawn_local_obj).
#[derive(Debug)]
pub struct CortexMExecutor {
    pool: FuturesUnordered<LocalFutureObj<'static, ()>>,
    incoming: Rc<Incoming>,
}

/// A handle to a [`CortexMExecutor`](CortexMExecutor) that implements
...
lazy_static! {
    static ref NOTIFY: Arc<WakeNotify> = Arc::new(WakeNotify {
        unparked: AtomicBool::new(false),
    });
}

impl ArcWake for WakeNotify {
    fn wake_by_ref(arc_self: &Arc<Self>) {
        ...
        if !unparked {
            ...
            // TODO: what do we do in this scenario? I don't think anything needs to be done
        }
    }
}
...
impl CortexMExecutor {
    /// Create a new, empty pool of tasks.
    pub fn new() -> CortexMExecutor {
        CortexMExecutor {
    ...
    ///
    /// ```
    /// use futures::executor::CortexMExecutor;
    ///
...
impl Default for CortexMExecutor {
    fn default() -> Self {
        Self::new()
    }
}
...
///
/// Use a [`CortexMExecutor`](CortexMExecutor) if you need finer-grained control over
/// spawned tasks.
...
impl<S: Stream + Unpin> Iterator for BlockingStream<S> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        CortexMExecutor::new().run_until(self.stream.next())
    }
